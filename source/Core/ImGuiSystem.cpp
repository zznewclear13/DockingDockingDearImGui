...
{
	ImGuiSystem::ExampleDescriptorHeapAllocator ImGuiSystem::g_pd3dSrvDescHeapAlloc;
	...
	bool ImGuiSystem::Initialize()
	{
		...
		if (!CreateDeviceD3D(AppContext::Get().m_systemWindow->GetWindowHandle()))
		{
			CleanupDeviceD3D();
		...
		ImGui_ImplWin32_Init(AppContext::Get().m_systemWindow->GetWindowHandle());
	...
	void ImGuiSystem::Update()
	{
		...
		if ((g_SwapChainOccluded && g_pSwapChain->Present(0, DXGI_PRESENT_TEST) == DXGI_STATUS_OCCLUDED) || ::IsIconic(AppContext::Get().m_systemWindow->GetWindowHandle()))
		{
			::Sleep(10);
		}
	...
	bool ImGuiSystem::CreateDeviceD3D(HWND hWnd)
	{
		// Setup swap chain
	...
	void ImGuiSystem::CleanupDeviceD3D()
	{
		CleanupRenderTarget();
	...
	void ImGuiSystem::CreateRenderTarget()
	{
		for (UINT i = 0; i < APP_NUM_BACK_BUFFERS; i++)
		{
	...
	void ImGuiSystem::CleanupRenderTarget()
	{
		WaitForLastSubmittedFrame();
	...
	void ImGuiSystem::WaitForLastSubmittedFrame()
	{
	...
	ImGuiSystem::FrameContext* ImGuiSystem::WaitForNextFrameResources()
	{
	...
	LRESULT WINAPI ImGuiSystem::WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
	{
